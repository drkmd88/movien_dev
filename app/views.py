...
def data_pre():
  ...
  """
    ...
    p = figure(plot_height=580, plot_width=800, title="",
...
def recomd():
    ...
    passValue["rating"] = [0]*4 if not hasRating else [x[1]*10/1.5 for x in recomds]
    passValue["prod"] = [0]*4 if not hasProd else [round(x[8]*15) for x in recomds]
    passValue["budget"] = [0]*4 if not hasBudget else [round(math.exp(x[2]*20))/1000000 for x in recomds]
    passValue["profit"] = [0]*4 if not hasProfit else [x[3]*1.8 for x in recomds]
